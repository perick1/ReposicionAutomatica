...
def plotRepoQuiebres(M, I0, F, RF, vT, tienda, sku):
    tienda  = tienda - 1
    sku     = sku - 1
    repo    = M[0 ,sku,tienda]
    inve    = M[1 ,sku,tienda]
    ...
    ax.plot(x,RD/max,color='g', alpha=0.8,label = 'Forecast con Ruido')
    #print(opt)
...
def plotResumen4(M, I0, F,Rf,vT, SKU, Ts):
    for i in SKU:
        for j in Ts:
            plotRepoQuiebres(Mvals, Minv, F,RF, vT, j, i)
...
def RuidoGRB(Tt ,vT,SKU ,Ts ,P ,C ,F ,SCD0 ,I0 ,Me ,Tr ,B ,Fvol, std):
    ...
    for sem in TT[:-(vT)+1]:
        ...
        for i in SKU:
            for j in Ts:
                RuidoF = round(np.random.normal(0, std * F[i,j][sem], 1)[0])
                #RuidoF = 0
                RuidoTsSKU[i,j] = RuidoF
                errRepo = abs(I0[i,j] + GRB_repo[sem][i,j] - F[i,j][sem])
    ...
    for sem in T:
        ...
        GRB_SCD[sem]    = {i : vals['SCD'][i,sem] for i in SKU}
        GRB_inve[sem]   = {(i,j) : vals['I'][i,j,sem] for i in SKU for j in Ts}
    return {'repo':GRB_repo ,'inventario': GRB_inve, 'SCD':  GRB_SCD, 'opt' :GRB_opt , 'venta': GRB_venta, 'Ruido': Ruido}

def curvaForcastRuido(dictR,F,SKU,Ts):
    Ttotal = len(dictR)
    Ndict = {}
    for i in SKU:
        for j in Ts:
...
output_vals =RuidoGRB(Nsemanas ,vT,SKU ,Ts ,P ,C ,F ,SCD0 ,I0 ,Me ,Tr ,B ,Fvol, 0.1)
...
RF = F
plotResumen4(Mvals, Minv, F,RF,vT, SKU, Ts)
'''
*****************************************************
                    Fin
*****************************************************
